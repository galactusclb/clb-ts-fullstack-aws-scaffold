...
const replicaAdapter = new PrismaPg(
    resolveDBConnection(process.env.RDS_READ_REPLICA_ENDPOINT ?? RDS_PROXY_ENDPOINT)
);
const readClient = new PrismaClient({ adapter: replicaAdapter });

const prisma = mainClient.$extends(readReplicas({ replicas: [readClient] })).$extends({
    query: {
        $allModels: {
            async $allOperations({ model, operation, args, query }) {
                return traceAsync(`db:${model}.${operation}`, () => query(args));
            },
        },
    },
});
